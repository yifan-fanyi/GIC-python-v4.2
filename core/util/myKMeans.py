...
def KKZ_init(X, n_clusters):
    ...
    for _ in range(n_clusters-1):
        dst = Cpredict(X, np.array(cluster_centers[-1]).reshape(1, -1), returnDist=True)
        dist.append(dst)
        idx = np.argmax(np.sum(np.array(dist).reshape(-1, X.shape[0]),axis=0))
        #print(np.sqrt(np.min(dst)), np.sqrt(np.max(dst)), np.min(euclidean_distances(np.array(cluster_centers), X[idx].reshape(1,-1))), np.max(euclidean_distances(np.array(cluster_centers), X[idx].reshape(1,-1))))
